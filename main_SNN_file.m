...
if size(mean_ret,1) ~= n_stocks || size(cov_mat,1) ~= n_stocks || size(cov_mat,2) ~= n_stocks
    error('Input data dimensions do not match after cleaning. Potential errors or bugs found.');
end

%% 3. Pre-clustering for Dimensionality Reduction
...
tic;
[optimal_weights_snn, selected_idx_snn, convergence_data_snn] = snn_returns_solver(mean_ret, cov_mat, params);
snn_time = toc;
...
ann_params = struct();
ann_params.n_epochs = 150;
ann_params.hidden_size = 16;
ann_params.learning_rate = 0.02;
ann_params.cardinality = [30, 50];
tic;
[optimal_weights_ann, selected_idx_ann, ann_info] = ann_returns_solver(mean_ret, cov_mat, ann_params);
ann_time = toc;
...
%% Create Plots
visualization(optimal_weights_snn, optimal_weights_ann, convergence_data_snn, ann_info, mean_ret, returns);
